...
'[<-.blockmatrix' <- function (M,i=1:nrow(M),j=1:ncol(M),value)  {
	...
	if (is.matrix(value)) {
		...
		value <- as.blockmatrix(value,nrow=1,ncol=1)
	} else if (is.numeric(value)) {
	
		if (length(i)!=1) warning("Replace of a numeric variable: i must be of length 1!")
		if (length(j)!=1) warning("Replace of a numeric variable: j must be of length 1!")
		i <- i[1]
		j <- j[1]
		
		value <- array(value,dim=dim(M[i,j]))
		value <- as.blockmatrix(value,nrow=1,ncol=1)
	...
	if (class(i)=="character") {
		...
		### ec 20151019
		mids <- unique(M$value[i,j])
	#	print(mids)
	#	print(names(value))
		mids <- mids[mids %in% names(value)]
		...
		warning("Not implemented for zero blockmatrix: nothing occurred!!")
	}
	#### cleaning ec 20151019
	
	vnames <- unique(as.vector(value(M)))
	vnames <- names(M)[names(M) %in% vnames]
	vnames <- c(vnames,"value")
	class(M) <- "list"
	M <- M[vnames]
	class(M) <- "blockmatrix"
